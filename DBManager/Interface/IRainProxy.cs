...
namespace Hydrology.DBManager.Interface
{
    ...
    public interface IRainProxy: IMultiThread
    {
        ...
        /// <returns></returns>
        List<CEntityRain> getListRainByTime(string stationid, DateTime start, DateTime end);

        /// <summary>
        /// 根据起止时间和站点ID获取雨量和水位数据
        /// </summary>
        /// <param name="stationid"></param>
        /// <param name="start"></param>
        /// <param name="end"></param>
        /// <returns></returns>
        List<CEntityRainAndWater> getRainAndWaterByTime(string stationid, DateTime start, DateTime end);
