...
namespace Hydrology.DBManager.DB.SQLServer
{
    public class CSQLStation : CSQLBase, IStationProxy
    {
        ...
        public void AddNewRow(CEntityStation entity)
        {
            ...
            row[CN_Dataprotocol] = entity.DataProtocol;
        ...
        public List<CEntityStation> QueryAll()
        {
            ...
            for (int rowid = 0; rowid < dataTableTmp.Rows.Count; ++rowid)
            {
                ...
                station.DataProtocol = dataTableTmp.Rows[rowid][CN_Dataprotocol].ToString();
        ...
        private String GetUpdateSQL(CEntityStation entity)
        {
            ...
            String strDataprotocol = entity.DataProtocol == null ? "null" : String.Format("'{0}'", entity.DataProtocol);
        ...
        public CEntityStation QueryById(string stationid)
        {
            ...
            if (dataTableTmp.Rows.Count == 1)
            {
                ...
                try
                {
                    if(dataTableTmp.Rows[0][CN_RainChange] != null && dataTableTmp.Rows[0][CN_RainChange].ToString() != "")
                    {
                        result.DRainChange = decimal.Parse(dataTableTmp.Rows[0][CN_RainChange].ToString());
                    }
                    else
                    {
                        result.DRainChange = 100;
                    }
                ...
                try
                {
                    if (dataTableTmp.Rows[0][CN_WaterBase] != null && dataTableTmp.Rows[0][CN_WaterBase].ToString() != "")
                    {
                    ...
                    if (dataTableTmp.Rows[0][CN_WaterChange] != null && dataTableTmp.Rows[0][CN_WaterChange].ToString() != "")
                    {
                        result.DWaterChange = decimal.Parse(dataTableTmp.Rows[0][CN_WaterChange].ToString());
                    }
                    else
                    {
                        result.DWaterChange = 10;
                    }
