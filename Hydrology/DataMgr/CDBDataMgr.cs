...
namespace Hydrology.DataMgr
{
    class CDBDataMgr
    {
        ...
        private Queue<CEntityWater> waterQueue;
        private Queue<CEntityWaterSpeed> waterSpeenQueue;

        public Dictionary<string, CEntityStation> m_mapGprsStation;    //站点gprs和站点映射

        public Dictionary<string, string> map_StationId;
        ...
        public bool Init()
        {
            ...
            map_StationId = new Dictionary<string, string>();
            waterQueue = new Queue<CEntityWater>();
            waterSpeenQueue = new Queue<CEntityWaterSpeed>();
...
#pragma warning restore CS0168 // 声明了变量“e”，但从未使用过
            ...
            //读取XML
            map_StationId = XmlHelper.getStationMapInfo();
            Protocol.Manager.XmlStationData.Instance.DeleteFile();
            Protocol.Manager.XmlStationData.Instance.Serialize(m_listStations);
        ...
        public void initZhongaoFile()
        {
            string path = XmlHelper.getXMLInfo();
            if (path != null && path != "" && path.Length > 0)
            {
                foreach (CEntityStation entity in m_listStations)
                {
                    string fileStr = path + "\\" + entity.StationName + ".Dat";
                    string file = @fileStr;
                    if (!File.Exists(file))
                    {
                        File.Create(file);
        ...
        public IWaterSpeedProxy GetWaterSpeedProxy()
        {
            return m_proxyWaterSpeed;
        }
...
#pragma warning restore CS0168 // 声明了变量“exp”，但从未使用过
        ...
        public void EHRecvSpeedDatas(object sender, CEventRecvStationDatasArgs args)
        {

            //m_mutexTaskList.WaitOne();
            NewTask(() => { DealSpeedDatas(args); });
            //DealRTDDatas(args);
            //m_mutexTaskList.ReleaseMutex();
        }
        ...
        public void EHRecvSM100HDatas(object sender, CEventRecvStationDatasArgs args)
        {

            //m_mutexTaskList.WaitOne();
            NewTask(() => { DealSM100HDatas(args); });
            //DealRTDDatas(args);
            //m_mutexTaskList.ReleaseMutex();
        }
            ...
            try
            {
                ...
                if (args.EStationType == EStationType.EHydrology || args.EStationType == EStationType.ERiverWater)
                {
                    ...
                    foreach (CSingleStationData data in args.Datas)
                    {
                        ...
                        {
                            CDBLog.Instance.AddInfo(station.StationID + "水位值位200或者-20000");
                            continue;
                        }
                        CEntityWater water = new CEntityWater();
                        water.StationID = station.StationID;
                    ...
                    m_proxyWater.batchInsertRows(listWater);
                }
                #endregion
                #region 符合条件的数据写进中澳格式数据
                //try
                //{
                //    string path = XmlHelper.getXMLInfo();
                //    //判定该系统是否需要写该文件
                ...
                //    {
                //        //判定是否是定时报
                //        if (args.EMessageType == EMessageType.ETimed)
                //        {
                //            string fileStr = path + "\\" + station.StationName + ".Dat";
                //            string file = @fileStr;

                //            //获取需要写的数据
                //            foreach (CSingleStationData data in args.Datas)
                //            {
                //                string hydroInfoText = string.Empty;

                //                hydroInfoText = hydroInfoText + "1,";
                //                hydroInfoText = hydroInfoText + data.DataTime.Year.ToString() + ",";
                //                hydroInfoText = hydroInfoText + getDayOfYear(data.DataTime).ToString() + ",";
                //                string hour = data.DataTime.Hour.ToString();
                //                if (hour.Length < 2)
                //                {
                //                    hour = "0" + hour;
                //                }
                //                string minute = data.DataTime.Minute.ToString();
                //                if (minute.Length < 2)
                //                {
                //                    minute = "0" + minute;
                //                }
                //                hydroInfoText = hydroInfoText + hour + minute + ",";
                //                hydroInfoText = hydroInfoText + station.StationID.ToString() + ",";
                //                if (data.WaterStage.HasValue && data.WaterStage >= 0)
                //                {
                //                    hydroInfoText = hydroInfoText + data.WaterStage.ToString() + ",";
                //                }
                //                else
                //                {
                //                    hydroInfoText = hydroInfoText + ",";
                //                }
                //                if (data.TotalRain.HasValue && data.TotalRain >= 0)
                //                {
                //                    decimal? totalRain = data.TotalRain * (decimal?)station.DRainAccuracy;
                //                    hydroInfoText = hydroInfoText + totalRain.ToString() + ",";
                //                }
                //                else
                //                {
                //                    hydroInfoText = hydroInfoText + ",";
                //                }
                //                if (data.Voltage.HasValue && data.Voltage > 0)
                //                {
                //                    hydroInfoText = hydroInfoText + data.Voltage.ToString();
                //                }
                //                else
                //                {
                //                    hydroInfoText = hydroInfoText + "12.85";
                //                }

                //                FileStream fs = new FileStream(file, FileMode.Append);
                //                StreamWriter sw = new StreamWriter(fs);
                //                sw.WriteLine(hydroInfoText);
                //                sw.Flush();
                //                sw.Close();
                //                fs.Close();
                //            }
                //        }
                //        CDBLog.Instance.AddInfo("写入中澳格式文件成功！");
                //    }
                //}
                //catch (Exception e)
                //{
                //    CDBLog.Instance.AddInfo("写入中澳格式文件失败！");
                //}
                #endregion
                // 更新水量表，雨量表以及电压表
                #region 雨量表
                ...
                {
                    if (args.Datas[tmpDataCount - 1].WaterStage != -20000 || args.Datas[tmpDataCount - 1].WaterStage != -200)
                    {
                        ...
                        if(realtime.DWaterYield < 0)
                        {
                            realtime.DWaterYield = null;
                        }
...
#pragma warning restore CS0168 // 声明了变量“e”，但从未使用过
        ...
        /// </summary>
        /// <param name="args"></param>
        private void DealSM100HDatas(CEventRecvStationDatasArgs args)
        {
            ...
            try
            {
                ...
                realtime.DV4 = args.Datas[tmpDataCount - 1].v4;
                realtime.DWaterFlowFindInTable = args.Datas[tmpDataCount - 1].Q;
                realtime.DWaterFlowActual = args.Datas[tmpDataCount - 1].Q2;
                realtime.Dvoltage = args.Datas[tmpDataCount - 1].Voltage;
                realtime.EIMessageType = args.EMessageType;
                realtime.EIChannelType = args.EChannelType;
