...
#pragma warning restore CS0649 // 从未对字段“CPortDataMgr._lastError”赋值，字段将一直保持其默认值 null
        ...
        public String SendAdjustClockFirst(CEntityStation station)
        {
            ...
            if (hdgprs != null)
            {
                ...
                if (hdgprs.FindByID(userID, out dtuID))
                {
                    ...
                    {
                        Debug.WriteLine("创建下行指令失败" + e.Message);
                    }
                }
                else
                {
                ...
                if (gname == "HD-GPRS")
                {
                    ...
                    foreach (var port in dll.Ports)
                    {
                        try
                        {
                            ...
                            if (hdgprs.DSStartService((ushort)port.PortNumber, protocol, 1, null, IntPtr.Zero) == 0)
                            {
                ...
                if (gname == "SX-GPRS")
                {
                    ...
                    foreach (var port in dll.Ports)
                    {
                        try
                        {
                            ...
                            //gprs.BatchSDDataReceived += CProtocolEventManager.BatchSDDataReceived;
        ...
        public bool SendHex(CEntityStation station)
        {
            ...
            else if (hdgprs != null)
            {
                ...
                if (hdgprs.FindByID(userID, out  dtuID))
                {
                    id = System.Text.Encoding.Default.GetString(dtuID);
                    hdgprs.sendHex(id.Trim(), bts, (uint)bts.Length, null);
                }
                return true;
            }
            return false;
        ...
        //        if (gsm != null)
        //        {
        //            //bool result = gsm.DeleteAll();
        //            if (result == false)
        //            {
        //                CSystemInfoMgr.Instance.AddInfo(string.Format("清空串口\"{0}\"......失败!", portNum));
        //            }
        //            else
        //            {
        //                gsm.UpDataReceived += CProtocolEventManager.UpDataReceived;
        ...
        //                }
        //                //gsm.OpenPort();
        //                CSystemInfoMgr.Instance.AddInfo(string.Format("清空串口\"{0}\"......完成!", portNum));
        //                gsm.InitGsm();
        //            }
        //        }
        //    }
        ...
        public void CloseAll()
        {
            try
            {
                ...
                if (this.m_gprsLists != null)
                {
                    foreach (var gprs in this.m_gprsLists)
                    {
                        CSystemInfoMgr.Instance.AddInfo("正在关闭SX-GPRS");
                        gprs.Close();
                        CSystemInfoMgr.Instance.AddInfo("关闭SX-GPRS成功");
                    }
                }
                //  关闭HDGPRS
                if (this.m_hdgprsLists != null)
                {
                    foreach (var hdgprs in this.m_hdgprsLists)
                    {
                        CSystemInfoMgr.Instance.AddInfo("正在关闭HD-GPRS");
                        hdgprs.Close();
                        CSystemInfoMgr.Instance.AddInfo("关闭HD-GPRS成功");
                    }
                }
                
                CSystemInfoMgr.Instance.AddInfo("关闭所有通讯成功");
            }
            catch (Exception exp) { Debug.WriteLine(exp.Message); }
        }
    }
...
#pragma warning restore CS0168 // 声明了变量“e8”，但从未使用过
            ...
            else if (str.Contains("@@SM100H"))
            {
                ...
                CDBDataMgr.Instance.EHRecvSM100HDatas(null, stationDatas);
            }
