...
namespace Hydrology.CControls
{
    ...
    class CDataGridViewGPRSConfig : CExDataGridView
    {
        ...
        public void InitDataSource()
        {
            ...
            //20170602
            List<IHDGprs> listHDGprs = CPortDataMgr.Instance.HDGprsLists;
            ...
            foreach (IGprs gprs in listGprs)
            {
                ...
                {
                    // 正在监听端口
                    listPortStarted.Add(gprs.GetListenPort());
                }
            }
            //20170602
            if(listHDGprs != null)
            {
                foreach (IHDGprs hdgprs in listHDGprs)
                {
                    //if (hdgprs.GetStarted())
                    //{
                    //    // 正在监听端口
                    listPortStarted.Add(hdgprs.GetListenPort());
                    // }
                }
            }
            //foreach (IHDGprs gprs in listHDGprs)
            //{
            //    //if (gprs.GetStarted())
            //   // {
            //        // 正在监听端口
            //        listPortStarted.Add(gprs.GetLi);
            //    }
            //}
            //20170602
        ...
        public override bool DoSave()
        {
            ...
            if (result)
            {
                ...
                for(int i =0;i < m_listPortConfig.Count; i++)
                {
                    ...
                    //if (m_listPortConfig[i].ProtocolChannelName == "SX-GPRS")
                    //{
                    //    CPortDataMgr.Instance.StartGprs(false);
                    //}
                    //if (m_listPortConfig[i].ProtocolChannelName == "HD-GPRS")
                    //{
                    //    //hdgprs默认启动udp
                    //    CPortDataMgr.Instance.StartHDGprs(false);
                    //}

                }

                //20170625 通讯协议和数据协议的对应
                //TODO
                //当通讯协议实现多端口接收的时候，将是端口号和数据协议对应
               
                StreamWriter sw = new StreamWriter("Config/map.txt",false, System.Text.Encoding.Default);
                for (int i = 0; i < m_listPortConfig.Count; i++)
                {
                    sw.WriteLine(m_listPortConfig[i].ProtocolChannelName + "," + m_listPortConfig[i].ProtocolDataName);       
                }
                sw.Close();
        ...
        protected void InitUI()
        {
            ...
            for (int i = 0; i < m_listPortConfig.Count; ++i)
            {
                ...
                else
                {
                    ...
                    CPortDataMgr.Instance.StopHDGprs();
                }

                if (null != m_listPortConfig[i].BTcpOrUdp)
                {
                    listShow.Add((m_listPortConfig[i].BTcpOrUdp.Value ? CS_UDPorTCP_TCP : CS_UDPorTCP_UDP));
                }
                else
                {
