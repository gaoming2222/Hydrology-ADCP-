...
namespace Hydrology.Forms
{
    public partial class CBatchFlashMgrForm : Form
    {
        ...
        private void Init()
        {
            ...
            CProtocolEventManager.BatchSDForUI += this.BatchSDForUI_EventHandler;
        ...
        private void BatchForUI_EventHandler(object sender, BatchEventArgs e)
        {
            ...
            if (result != null)
            {
                if (this.IsHandleCreated)
                {
                    ...
                    List<CTimeAndData> Datas = result.Datas;
                    List<CTimeAndData> Rains = result.Rains;
                    List<CTimeAndData> Waters = result.Waters;
                    ...
                    else if (this.radioBoard.Checked == true)
                    {
                        ...
                        if (Waters.Count != 0)
                        {
                            foreach (var d in Waters)
                            {
                                dateList.Add(d.Time);
                            }
                        }
                        DStartTime = dateList.Min();
                        DEndTime = dateList.Max();
                    }
                    else
                    {
                        DateTime tmp = this.dtp_StartTime.Value;
                    ...
                    if (radioSaveDB.Checked)
                    {
                        m_batchDataImport.Import(result, DStartTime, DEndTime);
                    }
                    //m_batchDataImport.Import(result, DStartTime, DEndTime);
                    List<CTimeAndData> datas = new List<CTimeAndData>();
                    if (radioSaveTxt.Checked)
                    {
                        if (result.StationType == EStationType.ERainFall || result.StationType == EStationType.EHydrology)
                        {
                            ...
                            sw.Write("站点类型: " + "雨量" + "\r\n");//写你的字符串。
                            datas = result.StationType == EStationType.ERainFall ? Datas : Rains;
                        ...
                        if (result.StationType == EStationType.ERiverWater || result.StationType == EStationType.EHydrology)
                        {
                            ...
                            sw.Write("接收数据 : " + rawData + "\r\n");//写你的字符串。
                            sw.Write("站点ID : " + result.StationID + "\r\n");//写你的字符串。
                            datas = result.StationType == EStationType.ERiverWater ? Datas : Waters;
                            sw.Write("站点类型: " + "水位" + "\r\n");//写你的字符串。
                    ...
                    this.listView1.Invoke((Action)delegate
                    {
                        ...
                        try
                        {
                            ...
                            else if (result.StationType == EStationType.EHydrology)
                            {
                                this.listView1.Items.Add(new ListViewItem()
                                {
                                    Text = "站点类型: " + "水文"
        ...
        private void BatchSDForUI_EventHandler(object sender, BatchSDEventArgs e)
        {
            ...
            if (result != null)
            {
                if (this.IsHandleCreated)
                {
                    if (radioSaveTxt.Checked)
                    {
                        string filePath = "BatchData\\SD.txt";//这里是你的已知文件
                        string strPath = Path.GetDirectoryName(filePath);
                        if (!Directory.Exists(strPath))
                        {
                            Directory.CreateDirectory(strPath);
                        ...
                        foreach (var item in result.Datas)
                        {
                            try
                            {
                                sw.Write("    时间：" + item.Time.ToString() + "    雨量：" + item.rain + "\r\n    水位：" + item.water + "    电压：" + item.voltage + "\r\n");
                            }
                            catch (Exception exp) { Debug.WriteLine(exp.Message); }
                        }
                        sw.Close();
                    ...
                    this.listView1.Invoke((Action)delegate
                    {
                        ...
                        foreach (var item in result.Datas)
                        {
                            try
                            {
                                this.listView1.Items.Add(new ListViewItem()
                                {
                                    Text = item.Time.ToString() + "    雨量：" + item.rain + "    水位：" + item.water + "    电压：" + item.voltage
                                });
                            }
                            catch (Exception exp) { Debug.WriteLine(exp.Message); }
                        }
        ...
        private void btnStartTrans_Click(object sender, EventArgs e)
        {
            ...
            ETrans trans = this.radioHour.Checked ? ETrans.ByHour : (this.radioDay.Checked ? ETrans.ByDay : ETrans.ByDayHour);
            ...
            if (radioSD.Checked)
            {
                ...
                string logMsg = String.Format("--------SD批量传输    目标站点（{0:D4}）--------- ", int.Parse(sid));
                // 写入系统日志
            ...
            else
            {
                if (radioFlash.Checked)
                {
                    ...
                    string logMsg = String.Format("--------Flash批量传输    目标站点（{0:D4}）--------- ", int.Parse(sid));
                    // 写入系统日志
                    CSystemInfoMgr.Instance.AddInfo(logMsg);
                    this.listView1.Items.Add(logMsg);
                ...
                else if (radioBoard.Checked)
                {
                    if (this.radioHour.Checked == true)
                    {
                        ...
                        if(ts.TotalDays > 5)
                        {
                            MessageBox.Show("主板传输不能超过5天!");
                            return;
                        }
                        if (ts.TotalDays < 0)
                        {
                            MessageBox.Show("主板传输不能超过今天!");
                            return;
                        }
                    }
                    ...
                    station.StationType = radioRain.Checked ? EStationType.ERainFall : EStationType.EHydrology;
                    string logMsg = String.Format("--------主板批量传输    目标站点（{0:D4}）--------- ", int.Parse(sid));
                    // 写入系统日志
                    CSystemInfoMgr.Instance.AddInfo(logMsg);
                    this.listView1.Items.Add(logMsg);
        ...
        private void RadioButton_CheckedChanged(object sender, EventArgs e)
        {
            ...
            {
                radioDayHour.Enabled = false;
                radioDay.Enabled = false;
                radioRain.Enabled = false;
                radioWater.Enabled = false;
                radioSD.Enabled = false;
            ...
            {
                radioDayHour.Enabled = true;
                radioDay.Enabled = true;
                radioRain.Enabled = true;
                radioWater.Enabled = true;
                radioSD.Enabled = true;
                dtp_StartTime.Enabled = true;
                dtp_EndTime.Enabled = true;

                if (radioSD.Checked)
                {
                    radioBoard.Enabled = false;
                    radioDay.Enabled = false;
                    radioDayHour.Enabled = false;
                    dtp_EndTime.Enabled = false;
                    radioSaveDB.Enabled = false;
                    // 小时选择模式
                    dtp_StartTime.CustomFormat = "yyyy-MM-dd HH";
                }
                else
                {
                    radioBoard.Enabled = true;
                    if (radioHour.Checked)
                    {
                        radioDay.Enabled = true;
                        radioDayHour.Enabled = true;
                        dtp_EndTime.Enabled = true;
                        radioSaveDB.Enabled = true;
                        // 小时选择模式
                        dtp_StartTime.CustomFormat = "yyyy-MM-dd HH";
                        dtp_EndTime.CustomFormat = "yyyy-MM-dd HH";
                    }
    ...
    class BatchDataImport
    {
        ...
        public void Import(CBatchStruct batch, DateTime DStartTime, DateTime DEndTime, bool isUpdate = false)
        {
            ...
            if (type == EStationType.ERainFall || type == EStationType.EHydrology)
            //if (type == EStationType.ERiverWater)
            // if (true)
            {
                if (type == EStationType.EHydrology) datas = batch.Rains;
                for (int i = 0; i < datas.Count; i++)
                {
                    ...
                    int RawtotalRain;
                    Nullable<Decimal> RawrainAccuracy;
                    try
                    {
                        RawtotalRain = int.Parse(data.Substring(0, 4));
                        RawrainAccuracy = decimal.Parse((data.Substring(4, 2)));
                    }
                    catch (Exception)
                    {
                        continue;
            ...
            //操作water表
            if (type == EStationType.ERiverWater || type == EStationType.EHydrology)
            //if (type == EStationType.ERainFall)
            {
                if (type == EStationType.EHydrology) datas = batch.Waters;
...
#pragma warning restore CS0168 // 声明了变量“e”，但从未使用过
                ...
                m_proxyWater.batchInsertRows(waterResults);
